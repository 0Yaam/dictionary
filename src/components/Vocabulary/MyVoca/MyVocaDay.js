...
const MyVocaDay = () => {
  ...
  const words = useMemo(() => {
    ...
    return pairs.slice(start, end).map(([english, vietnamese]) => ({ english, vietnamese }));
  }, [dayNum, pairs]);

  const handleNext = () => {
    const next = Math.min(dayNum + 1, totalDays);
    if (next !== dayNum) navigate(`/vocabulary/MyVoca/VocaNgay/${next}`);
  };

  const handlePrevious = () => {
    const prev = Math.max(dayNum - 1, 1);
    if (prev !== dayNum) navigate(`/vocabulary/MyVoca/VocaNgay/${prev}`);
  };
    ...
    <>
      ...
      <div className="w-full h-screen flex justify-center bg-gradient-to-b from-white to-[#fff6ee] relative">
        <button
          ...
          className={`absolute left-[calc(44%-300px)] top-[calc(44%-300px)] ${dayNum > 1 ? "bg-gray-300 hover:bg-gray-400" : "bg-gray-200"} text-gray-700 w-[100px] h-[45px] rounded-md shadow-md flex justify-center items-center font-bold`}
          disabled={dayNum <= 1}
        >
          Previous
        </button>
              ...
              {words.map((word, index) => (
                <div key={index} className="flex items-center bg-white p-3 rounded-lg shadow-md border border-gray-200">
                  ...
                  <div className="flex-1 text-right">
                    <span className="text-[#743131] font-medium pr-9">{word.vietnamese}</span>
                  </div>
                </div>
              ))}
            </div>
