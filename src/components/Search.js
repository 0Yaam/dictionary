...
class DictionaryTree {
  ...
  loadFromJson(data) {
    data.forEach(entry => {
      this.insert(entry.word, entry.definition, entry.ipa);
    });
  }
}
...
const Search = ({ word, setWord }) => {
  ...
  const searchHandler = () => {
    ...
    if (result) {
      setMeaning(result.meaning || null);
  ...
  return (
    <div className="max-w-3xl mx-auto my-2">
      ...
      <div className="mt-4 text-lg text-left">
        {searched && (
          <div>
            <strong>Kết quả:</strong> {meaning || "Từ không có trong từ điển!"}
          </div>
        )}
      </div>
